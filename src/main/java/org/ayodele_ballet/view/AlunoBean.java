...
import org.ayodele_ballet.model.Aluno;
...
public class AlunoBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Aluno> root) {
		...
		String nome_responsavel = this.example.getResponsavel().getNome();
		if (nome_responsavel != null && !"".equals(nome_responsavel)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("nome_responsavel")),
					'%' + nome_responsavel.toLowerCase() + '%'));
		}
		String local_aula = this.example.getLocal_aula();
		if (local_aula != null && !"".equals(local_aula)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("local_aula")),
					'%' + local_aula.toLowerCase() + '%'));
		}
		String nivel = this.example.getNivel();
		if (nivel != null && !"".equals(nivel)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("nivel")),
					'%' + nivel.toLowerCase() + '%'));
		}
